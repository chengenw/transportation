...
class BaseSynthesizer:
    ...
    def fit_processed_data(self, processed_data, processed_data_base=None, args=None):
        ...
        if not processed_data.empty:
            if processed_data_base is None:
                self._fit(processed_data, args=args)
            else:
                self._fit(processed_data, processed_data_base, args=args)
    ...
    def fit(self, data, base_data=None, args=None):
        ...
        self._random_state_set = False
        processed_data = self._preprocess(data) #$ rdt reversible transformation
        if base_data is None:
            self.fit_processed_data(processed_data, args=args)
        else:
            cprint(f'base data use a different preprocessing... e.g., TVAE vs CTGAN', 'yellow')
            processed_data_base = self.synthesizer_base._preprocess(base_data) #$ the same data processor??
            self.fit_processed_data(processed_data, processed_data_base, args=args)
...
class BaseSingleTableSynthesizer(BaseSynthesizer):
    ...
    def _sample_rows(self, num_rows, conditions=None, transformed_conditions=None,
                     float_rtol=0.1, previous_rows=None, keep_extra_columns=False, previous_rows_base=None):
        ...
        if self._model and need_sample:
            ...
            if not hasattr(self, 'synthesizer_base') or self.synthesizer_base is None:
                sampled = self._data_processor.reverse_transform(raw_sampled)
            else:
                sampled_base = self.synthesizer_base._data_processor.reverse_transform(raw_sampled[0])
                sampled = self._data_processor.reverse_transform(raw_sampled[1])
            ...
            if previous_rows is not None:
                ...
                if previous_rows_base is not None:
                    sampled_base = pd.concat([previous_rows_base, sampled_base], ignore_index=True)

            sampled = self._data_processor.filter_valid(sampled)
            if hasattr(self, 'synthesizer_base') and self.synthesizer_base is not None:
                sampled_base = self.synthesizer_base._data_processor.filter_valid(sampled_base)
            ...
            if previous_rows_base is not None:
                sampled = pd.concat([sampled_base, sampled], axis=1)
    ...
    def _sample_batch(self, batch_size, max_tries=100,
        ...
        sampled_base = pd.DataFrame() if hasattr(self, 'synthesizer_base') and self.synthesizer_base is not None else None
