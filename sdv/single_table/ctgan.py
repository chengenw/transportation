...
import numpy as np
import pandas as pd
from ctgan import CTGAN, TVAE, HCTGAN, HTVAE
...
class CTGANSynthesizer(LossValuesMixin, BaseSingleTableSynthesizer):
    ...
    def __init__(self, metadata, enforce_min_max_values=True, enforce_rounding=True,
                 ...
                 pac=10, cuda=True, writer=None, synthesizer_base=None):
    ...
    def _fit(self, processed_data, processed_data_base=None, args=None):
        ...
        if processed_data_base is None:
            ...
            self._model.fit(processed_data, discrete_columns=discrete_columns, args=args)
        else:
            _validate_no_category_dtype(processed_data_base)

            # transformers_base = self._data_processor._hyper_transformer.field_transformers #$ same _data_processor??
            # metadata_base.validate()
            # discrete_columns_base = detect_discrete_columns(
            #     metadata_base,
            #     processed_data_base,
            #     transformers_base
            # )

            transformers_base = self.synthesizer_base._data_processor._hyper_transformer.field_transformers
            discrete_columns_base = detect_discrete_columns(
                self.synthesizer_base.get_metadata(),
                processed_data_base,
                transformers_base
            )
            self._model = HCTGAN(**self._model_kwargs)
            self._model.fit(processed_data, discrete_columns=discrete_columns, base_data=processed_data_base, base_d_cols=discrete_columns_base, synthesizer_base=self.synthesizer_base, args=args)
    ...
    def _sample(self, num_rows, conditions=None):
        ...
        if conditions is None:
            ...
            # if self.synthesizer_base is None:
            #     return self._model.sample(num_rows)
            # else:
            #     return self._model.sample(num_rows, synthesizer_base=self.synthesizer_base)
...
class TVAESynthesizer(LossValuesMixin, BaseSingleTableSynthesizer):
    ...
    def __init__(self, metadata, enforce_min_max_values=True, enforce_rounding=True,
                 ...
                 l2scale=1e-5, batch_size=500, epochs=300, loss_factor=2, cuda=True, writer=None, synthesizer_base=None, **kwargs):
    ...
    def _fit(self, processed_data, processed_data_base=None, args=None):
        ...
        if processed_data_base is None:
            ...
            self._model = HTVAE(**self._model_kwargs)
            self._model.fit(processed_data, discrete_columns=discrete_columns, base_data=processed_data_base,
                            base_d_cols=discrete_columns_base, synthesizer_base=self.synthesizer_base, args=args)
